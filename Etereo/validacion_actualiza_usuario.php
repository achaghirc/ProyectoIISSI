<?php
        ...
        try{
            ...
            $_SESSION["errores"] = "<p>ERROR en la validacion: fallo en el acceso a la base de datos.</p><p>"
                .$e -> getMessage() ."</p>";
                Header("Location: form_actualizar_usuario.php?var2=" .$_SESSION["cif"]);
        }

        if(count($errores)>0){
            //Guardo en la sesion los mensajes de error y volvemos al formulario
            $_SESSION["errores"] = $errores;
            Header("Location:adminPage.php?var2=" .$_SESSION["cif"]);
        }else {
            //Si todo va bien, vamos a la pagina de exito
            Header("Location:accion_actualizar_usuario.php?var2=" .$_SESSION["cif"]);
        }
